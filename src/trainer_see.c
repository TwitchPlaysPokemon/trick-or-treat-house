...
bool8 CheckForTrainersWantingBattle(void)
{
    ...
    for (i = 0; i < EVENT_OBJECTS_COUNT; i++)
    {
        ...
        if (!gEventObjects[i].active)
            continue;
        if (gEventObjects[i].trainerType != TrainerType_Normal && gEventObjects[i].trainerType != TrainerType_SeeAllDirs)
            continue;
...
static u8 GetTrainerApproachDistance(struct EventObject *trainerObj)
{
    ...
    if (trainerObj->trainerType == TrainerType_Normal)  // can only see in one direction
    {
...
static bool8 WaitTrainerExclamationMark(u8 taskId, struct Task *task, struct EventObject *trainerObj)
{
    ...
    else
    {
        ...
        if (trainerObj->movementType == MOVEMENT_TYPE_TREE_DISGUISE || trainerObj->movementType == MOVEMENT_TYPE_MOUNTAIN_DISGUISE)
            task->tFuncId = 6;
        if (trainerObj->movementType == MOVEMENT_TYPE_HIDDEN || trainerObj->movementType == MOVEMENT_TYPE_HIDDEN2)
            task->tFuncId = 8;
        return TRUE;
    }
}
