...
static const u16 sHoennTrainerCardNeutralBg_Pal[] = INCBIN_U16("graphics/trainer_card/neutral_bg.gbapal");
...
static const u8 sTrainerPicFacilityClass[][GENDER_COUNT] = 
{
    [CARD_TYPE_FRLG] = 
    {
        [GENDER_M] = FACILITY_CLASS_RED, 
        [GENDER_F] = FACILITY_CLASS_LEAF,
        [GENDER_N] = FACILITY_CLASS_LEAF,
    }, 
    [CARD_TYPE_RS] = 
    {
        [GENDER_M] = FACILITY_CLASS_RS_BRENDAN, 
        [GENDER_F] = FACILITY_CLASS_RS_MAY,
        [GENDER_N] = FACILITY_CLASS_TREEKID, 
    }, 
    [CARD_TYPE_EMERALD] = 
    {
        [GENDER_M] = FACILITY_CLASS_BRENDAN, 
        [GENDER_F] = FACILITY_CLASS_MAY,
        [GENDER_N] = FACILITY_CLASS_TREEKID, 
    }
};
...
static bool8 LoadCardGfx(void)
{
    ...
    case 4:
        if (sData->cardType != CARD_TYPE_FRLG)
            LZ77UnCompWram(gEmeraldTrainerCard_Gfx, sData->cardTiles);
        else
            LZ77UnCompWram(gFireRedTrainerCard_Gfx, sData->cardTiles);
        break;
    case 5:
        if (sData->cardType == CARD_TYPE_FRLG)
...
static void BufferLinkPokeblocksNum(void)
{
    if (sData->cardType != CARD_TYPE_FRLG && sData->trainerCard.pokeblocksWithFriends)
    {
        ...
        StringExpandPlaceholders(sData->textNumLinkPokeblocks, gText_Var1DarkGreyShadowLightGrey);
    }
}
...
static void BufferBattleFacilityStats(void)
{
    ...
    case CARD_TYPE_RS:
        if (sData->hasBattleTowerWins)
        {
            ...
            StringExpandPlaceholders(sData->textBattleFacilityStat, gText_WSlashStraightSlash);
        }
        break;
    case CARD_TYPE_EMERALD:
        if (sData->trainerCard.frontierBP)
        {
            ...
            StringExpandPlaceholders(sData->textBattleFacilityStat, gText_Var1DarkLightGreyBP);
        }
        break;
    case CARD_TYPE_FRLG:
        break;
...
static u8 SetCardBgsAndPals(void)
{
    ...
    case 2:
        if (sData->cardType != CARD_TYPE_FRLG)
        {
            ...
            switch (sData->trainerCard.gender)
            {
                default:
                case GENDER_M:
                    // Included in the already loaded palette
                    break;
                case GENDER_F:
                    LoadPalette(sHoennTrainerCardFemaleBg_Pal, 16, 32);
                    break;
                case GENDER_N:
                    LoadPalette(sHoennTrainerCardNeutralBg_Pal, 16, 32);
                    break;
            }
        }
        else
